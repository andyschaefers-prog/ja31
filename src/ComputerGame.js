...
export default function ComputerGame({ profile, setProfile, stake, onExit }) {
  ...
  async function runBots(nextRound) {
    setBotsPlaying(true);
    ...
    for (let index = 1; index < totalPlayers; index += 1) {
      ...
      if (botKnocker === null && (aggressiveKnock || normalKnock)) botKnocker = index;
    }
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.game}>
    ...
    <View style={styles.alert}><Text style={styles.alertText}>{finished ? 'RUNDE ENTSCHIEDEN!' : botsPlaying ? '🤖 COMPUTERZUG – GENAU HINSCHAUEN!' : '⚡ DU BIST DRAN – KEINE ZEIT VERLIEREN!'}</Text></View>
