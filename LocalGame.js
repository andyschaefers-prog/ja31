...
export default function LocalGame({ onExit }) {
  ...
  if (!round) return <SafeAreaView style={styles.page}><View style={styles.setup}>
    <TouchableOpacity onPress={onExit}><Text style={styles.back}>‹ HAUPTMENÜ</Text></TouchableOpacity>
    <Text style={styles.title}>LOKALE RUNDE</Text><Text style={styles.subtitle}>Ein Handy. Bis zu vier große Klappen.</Text>
  ...
  if (!revealed) return <SafeAreaView style={styles.page}><View style={styles.pass}>
    <Text style={styles.passSmall}>HANDY WEITERGEBEN AN</Text><Text style={styles.passName}>{players[active].name}</Text>
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.game}>
    <View style={styles.top}><View><Text style={styles.player}>{players[active].name}</Text><Text style={styles.meta}>Level {players[active].level} · ◉ {players[active].coins} · {'●'.repeat(players[active].lives)}</Text></View><Text style={styles.logo}>JA 31</Text></View>
    {knocker !== null && <Text style={styles.warning}>{players[knocker].name} HAT GEKLOPFT · LETZTER ZUG</Text>}
    <Text style={styles.label}>MITTE</Text><View style={styles.cards}>{round.middle.map((card, index) => <Card key={card.id} card={card} onPress={() => swapMiddle(index)} />)}</View>
...
const styles = StyleSheet.create({
  page:{flex:1,backgroundColor:'#080808'},setup:{flexGrow:1,justifyContent:'center',padding:26},game:{padding:20,paddingBottom:40},pass:{flex:1,justifyContent:'center',padding:28},back:{color:'#999',fontWeight:'800',marginBottom:30},title:{color:'#fff',fontSize:38,fontWeight:'900',textAlign:'center'},subtitle:{color:RED,textAlign:'center',fontWeight:'800',marginBottom:40},label:{color:'#777',textAlign:'center',fontSize:11,letterSpacing:2,marginTop:20},countRow:{flexDirection:'row',justifyContent:'center',gap:12,marginVertical:18},count:{width:64,height:64,borderRadius:32,borderColor:'#444',borderWidth:1,alignItems:'center',justifyContent:'center'},active:{backgroundColor:RED,borderColor:RED},countText:{color:'#fff',fontSize:24,fontWeight:'900'},info:{color:'#888',textAlign:'center',marginVertical:18,lineHeight:20},primary:{backgroundColor:RED,padding:18,borderRadius:5,marginTop:12},outline:{borderColor:'#555',borderWidth:1,padding:17,borderRadius:5,marginTop:12},buttonText:{color:'#fff',fontWeight:'900',textAlign:'center',letterSpacing:1},passSmall:{color:'#777',textAlign:'center',letterSpacing:2},passName:{color:'#fff',textAlign:'center',fontSize:42,fontWeight:'900',marginTop:8},top:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'},player:{color:'#fff',fontSize:17,fontWeight:'900'},meta:{color:'#f1bd36',marginTop:4},logo:{color:RED,fontWeight:'900',fontSize:23},warning:{color:'#fff',backgroundColor:RED,textAlign:'center',fontWeight:'900',padding:10,marginTop:20},cards:{flexDirection:'row',justifyContent:'center',gap:9,marginVertical:12},card:{width:88,height:122,backgroundColor:'#f4f0e8',borderRadius:8,padding:9,borderWidth:3,borderColor:'#f4f0e8'},selected:{borderColor:RED,transform:[{translateY:-8}]},rank:{color:'#111',fontSize:24,fontWeight:'900'},suit:{color:'#111',fontSize:36,textAlign:'center',marginTop:10},red:{color:RED},resultRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',backgroundColor:'#151515',padding:15,marginTop:10,borderLeftColor:'#444',borderLeftWidth:4},winner:{borderLeftColor:RED},score:{color:'#fff',fontWeight:'900',fontSize:28},
});
